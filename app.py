...
def save_to_supabase(data, raw_text):
    """保存解析结果到数据库，返回成功记录数，出错时返回0并显示错误"""
    if not data:
        st.warning("没有数据可保存")
        return 0
    records = []
    ...
    if not records:
        ...
        return 0
    try:
        st.write("准备插入记录：", records)  # 调试输出
        response = supabase.table("price_records").insert(records).execute()
        st.write("Supabase 响应：", response)  # 调试输出
...
def extract_remark(line):
    """
    从一行文本中提取盒况和袋子信息，输出组合字符串。
    规则：
    - 盒况：好盒、压盒、瑕疵、盒损、破损、烂盒、破盒
    - 袋子：纸袋、M袋、礼袋、礼品袋、M号袋、S袋、+袋、带袋、有袋、无袋
    输出格式：如果两者都有，则 "盒况+有袋"；只有盒况则 "盒况"；只有袋子则 "有袋"；无则 ""
    """

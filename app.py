...
def extract_by_regex(line):
    ...
    clean_line = re.sub(r'(顺丰|普快|包邮|浙江|广东|山东|江苏|北京|河北|四川|安徽|福建|辽宁|河南|天津|重庆|黑龙江|吉林|上海|贵州|海南|山西|陕西|内蒙古|收货|发|地址|好盒|压盒|微瑕|件|个|×|x|叉|代中代|得物|闲鱼|飞机盒|LG-|LG)', '', line, flags=re.I)
    clean_line = re.sub(r'[\*]', ' ', clean_line)

    # 🔥 修复：现在能识别 LG-10434、10434小猪、粘连数字
    model_matches = re.findall(r'([1-9]\d{4})', clean_line)
    if not model_matches:
        return None, None, None
    model = model_matches[0]

    all_nums = [n for n in re.findall(r'\d+', clean_line) if n != model]
    if not all_nums:
        return None, None, None

    candidates = sorted([int(n) for n in all_nums], reverse=True)
    price = None
    for p in candidates:
        if 10 <= p <= 5000:
            price = p
...
with st.expander("📝 批量录入", expanded=True):
    ...
    if st.button("🔍 解析", type="primary", use_container_width=True):
        ...
        for key, item in unique_batch.items():
            ...
            # 🔥 你要的规则：同一天相同才去重，隔天可录入
            if m in latest_history:
                last_p = latest_history[m]["price"]
                last_r = latest_history[m]["remark"]
                last_time_str = latest_history[m].get("time", "")
                try:
                    last_date = pd.to_datetime(last_time_str).date()
                    if last_date == today and p == last_p and r.strip() == last_r.strip():
                        skip = True
                except:

...
with st.expander("📝 批量录入", expanded=True):
    ...
    txt = st.text_area("粘贴内容", height=200, value=st.session_state.input_text, key="input_area")
    ...
    if st.button("📌 识别粘贴并保存", type="primary", use_container_width=True):
        ...
        if save_list:
            ...
            st.success(f"✅ 已保存 {len(save_list)} 条有效数据")
            get_clean_data.clear()
        
        # 5. 把剪贴板内容重新粘贴回输入框
        st.session_state.input_text = clipboard_content
        st.rerun()

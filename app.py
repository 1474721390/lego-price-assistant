...
with st.expander("📝 批量录入", expanded=True):
    ...
    txt = st.text_area("粘贴内容", height=200, value=st.session_state.clipboard_content)

    # 新增：剪贴板读取 JS
    paste_js = """
    <script>
    navigator.clipboard.readText().then(text => {
        window.parent.postMessage({type: 'streamlit:setSessionState', key: 'clipboard_content', value: text}, '*');
    });
    </script>
    """
    ...
    # 2. 📌 识别粘贴并保存（自动读剪贴板 → 自动填 → 自动存）
    if st.button("📌 识别粘贴并保存", type="primary"):
        # 自动读取剪贴板并填入
        st.components.v1.html(paste_js, height=0)
        st.rerun()

    # 如果剪贴板有内容，自动解析+保存
    if st.session_state.clipboard_content:
        content = st.session_state.clipboard_content
        ...
        if save_list:
            save_batch(save_list)
            st.success(f"✅ 从剪贴板自动保存 {len(save_list)} 条成功！")
            get_clean_data.clear()
        # 用完清空
        st.session_state.clipboard_content = ""
        st.rerun()

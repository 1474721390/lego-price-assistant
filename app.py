...
def update_price_record(record_id, new_model, new_price):
...
# ---------- 整体趋势分析 ----------
def calculate_overall_trend(df_model):
    if len(df_model) < 3:
        return None
    recent = df_model.tail(min(7, len(df_model)))
    base_time = recent['时间'].min()
    x = (recent['时间'] - base_time).dt.total_seconds() / (3600 * 24)
    y = recent['价格']
    if len(x) < 2:
        return None
    slope = (x * y).sum() - (x.sum() * y.sum()) / len(x)
    slope /= (x * x).sum() - (x.sum() ** 2) / len(x)
    return slope
...
def get_trend_alerts():
    ...
    for model, group in grouped:
        if len(group) < 3:
            continue
        slope = calculate_overall_trend(group)
        if slope is None:
            continue
        if slope > 5:
            alerts.append((model, "上涨", slope))
        elif slope < -5:
            alerts.append((model, "下跌", slope))
    return alerts
...
if alerts:
    with st.container():
        ...
        for model, trend, slope in alerts:
            if trend == "上涨":
                st.warning(f"📈 型号 {model} 近期价格整体上涨（斜率 {slope:.2f} 元/天），请注意！")
            else:
                st.error(f"📉 型号 {model} 近期价格整体下跌（斜率 {slope:.2f} 元/天），请注意！")
        st.markdown("---")
...
if all_records_df.empty:
    ...
    # 型号搜索框
    search_term = st.text_input("🔍 搜索型号（支持模糊匹配）", placeholder="输入型号关键字，如 10320")
    if search_term:
        filtered_df = all_records_df[all_records_df['model'].astype(str).str.contains(search_term, na=False)]
    else:
    ...
    if filtered_df.empty:
        ...
        # 显示每条记录，并提供编辑和删除功能
        st.caption("以下为匹配的历史记录，可直接修改型号/价格，或点击删除按钮移除记录。修改后请点击“保存所有修改”。")
        # 创建一个副本用于编辑
        edit_data = filtered_df[['id', 'model', 'price', 'time']].copy()
        edit_data['time'] = pd.to_datetime(edit_data['time']).dt.strftime('%Y-%m-%d %H:%M')
        # 使用 data_editor 允许批量编辑
        edited_df = st.data_editor(edit_data, num_rows="dynamic", key="history_edit")
        # 添加删除按钮列
        # 由于 data_editor 不支持直接添加按钮，我们单独为每行生成一个删除按钮（使用循环）
        # 为了简单，我们增加一个“删除”列，用户点击按钮时删除该行。
        # 但 Streamlit 的按钮会触发整个脚本重运行，需要记录要删除的 ID。
        # 更简单：在 data_editor 中，用户可以删除行（通过点击行首的“-”），删除的行会从 edited_df 中消失，
        # 我们可以通过比较原始 ID 和编辑后的 ID 来确定哪些被删除。
        # 但为了用户友好，我们额外加一个复选框列？太复杂。
        # 采用另一种方法：在 data_editor 旁边放置一个“删除所选”按钮，让用户勾选要删除的行。
        # 最简单：在 data_editor 中增加一列“删除”，用户填写“是”，然后保存时处理。
        # 这里使用复选框方式：
        if 'delete_flag' not in edited_df.columns:
            edited_df['删除'] = False
        edited_with_checkbox = st.data_editor(edited_df, column_config={
            "删除": st.column_config.CheckboxColumn("删除", default=False)
        }, num_rows="dynamic", key="history_edit_with_del")
        
        if st.button("✅ 保存所有修改并删除选中记录"):
            # 处理删除
            to_delete = edited_with_checkbox[edited_with_checkbox['删除'] == True]['id'].tolist()
            for rid in to_delete:
                delete_price_record(rid)
            # 处理更新（除了删除的行）
            for idx, row in edited_with_checkbox.iterrows():
                if row['删除']:
                    continue
                record_id = row['id']
                ...
                if not original.empty:
                    old_model = str(original.iloc[0]['model'])
                    old_price = int(original.iloc[0]['price'])
                    if new_model != old_model or new_price != old_price:
                        update_price_record(record_id, new_model, new_price)
                        # 同时将该修正作为一个纠错案例（原始文本无法获取，但可以构造一个虚拟文本）
                        # 这里不自动生成纠错案例，因为原始文本缺失，但用户仍可手动添加纠错。
            st.success("历史数据已更新！")
            st.rerun()
...
# 将输入框和选择框放在同一行，实现“同一个框”的效果
col_left, col_right = st.columns(2)
with col_left:
    model_input = st.text_input("直接输入型号编号（如10320）", key="trend_input")
with col_right:
    models = get_model_list()
    if models:
        selected_model = st.selectbox("或从已记录型号中选择", options=[""] + models, index=0, key="trend_select")
    else:
        selected_model = ""
        st.info("暂无已记录型号")
...
if model_input and model_input.strip():
    query_model = model_input.strip()
elif selected_model and selected_model != "":

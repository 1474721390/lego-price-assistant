...
def extract_by_regex(line):
    ...
    # 匹配 LG-10434 或直接 10434
    model_pattern = r"(?:LG-)?(\d{5})"
...
# ------------------------------
# 批量录入 —— 【修复 1：剪贴板自动粘贴到输入框】
# ------------------------------
with st.expander("📝 批量录入", expanded=True):
    if "clip_content" not in st.session_state:
        st.session_state.clip_content = ""
    ...
    txt = st.text_area("粘贴内容", height=200, value=st.session_state.clip_content)
    ...
    # 📌 识别粘贴并保存（修复：自动读取 → 自动填入）
    if st.button("📌 识别粘贴并保存", type="primary"):
        c = st.components.v1.html(paste_script, height=0)
        if c:
            st.session_state.clip_content = c
            st.rerun()

    # 自动解析保存
    if st.session_state.clip_content:
        content = st.session_state.clip_content
        ...
        if save_list:
            save_batch(save_list)
            st.success(f"✅ 剪贴板自动保存 {len(save_list)} 条！")
            get_clean_data.clear()
        st.session_state.clip_content = ""
        st.rerun()

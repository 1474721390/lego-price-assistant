...
def get_trend_alerts():
    ...
    for model in models:
        ...
        if abs(change) > 5:
            if change > 0:
                alerts.append((model, "上涨", change))
            else:
                alerts.append((model, "下跌", change))
    return alerts
...
# ---------- 组合控件函数 ----------
def model_selector(key_prefix, label="型号选择"):
...
if all_records_df.empty:
    ...
    selected_model_for_history = model_selector("history", "选择要修改的型号")

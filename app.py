...
def get_trend_data():
    ...
    if 'remark' not in df.columns:
        df['remark'] = ''
    st.write(f"get_trend_data 返回 {len(df)} 行")  # 调试
    return df
...
def get_all_price_records():
    ...
    if 'remark' not in df.columns:
        df['remark'] = ''
    st.write(f"get_all_price_records 返回 {len(df)} 行")  # 调试
    return df
...
def get_model_list():
    ...
    st.write(f"get_model_list 返回 {len(models)} 个型号: {models}")  # 调试
    return models

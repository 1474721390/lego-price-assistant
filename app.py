...
def llm_verify(model, price, remark, line):
    ...
    # 自动通过条件：价格正常、型号正确、波动不大 → 直接通过，不调用 AI
...
with st.expander("📝 批量录入", expanded=True):
    ...
    if st.button("🔍 解析", type="primary", use_container_width=True):
        ...
        if not all_records.empty:
            ...
            all_records['china_date'] = all_records['time_dt'].dt.tz_convert(china_tz).dt.date
            today_mask = all_records['china_date'] == today

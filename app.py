...
def extract_by_regex(line):
    ...
    clean_line = re.sub(r'(顺丰|普快|包邮|浙江|广东|山东|江苏|北京|河北|四川|安徽|福建|辽宁|河南|天津|重庆|黑龙江|吉林|上海|贵州|海南|山西|陕西|内蒙古|收货|发|地址|好盒|压盒|微瑕|件|个|×|x|叉|代中代|得物|闲鱼|飞机盒)', '', line, flags=re.I)
    clean_line = re.sub(r'[\*]', ' ', clean_line)

    model_matches = re.findall(r'(?<!\d)([1-9]\d{4})(?!\d)', clean_line)
...
# ==================== AI校验：异常价格 + 波动超50% 自动复核 ====================
def llm_verify(model, price, remark):
    if is_price_abnormal(price):
        pass
    else:
        ...
        if model in latest:
            ...
            if last_price > 0:
                change_rate = abs(price - last_price) / last_price
                if change_rate <= 0.5:
                    return True, "正常"
        return True, "正常"

    prompt = f"型号:{model} 价格:{price} 备注:{remark}\n判断是否为真实有效乐高收购价，排除数量、单号等干扰，仅返回JSON格式：{{\"is_valid\":true/false,\"reason\":\"原因\"}}"
...
with st.expander("📝 批量录入", expanded=True):
    ...
    if st.button("🔍 解析", type="primary", use_container_width=True):
        ...
        for key, item in unique_batch.items():
            ...
            if skip:
                res.append({"型号":m,"价格":p,"备注":r,"原始":raw,"状态":"⏭️ 已跳过（重复）"})
                continue

...
# ---------- 组合控件（两个独立控件互斥） ----------
def dual_model_selector(key_prefix, label_text="输入型号", label_select="或从列表选择"):
    """
    返回用户选择的型号（字符串）。同时提供一个文本输入框和一个下拉选择框，两者互斥。
    """
    col1, col2 = st.columns(2)
    with col1:
        manual_input = st.text_input(label_text, key=f"{key_prefix}_manual", placeholder="例如 10320")
    with col2:
        options = get_model_list()
        if options:
            selected = st.selectbox(label_select, options=[""] + options, key=f"{key_prefix}_select")
        else:
            selected = ""
    # 互斥逻辑：如果手动输入不为空，则忽略下拉选择；否则使用下拉选择
...
with st.expander(f"⚠️ 价格趋势预警（{len(alerts)}个型号）", expanded=False):
    if alerts:
        ...
        # 使用双控件选择型号
        filter_model = dual_model_selector("alert_filter", "筛选型号（可输入）", "或从预警列表选择")
        filtered = alerts
        if filter_model:
...
if all_records_df.empty:
    ...
    selected_model = dual_model_selector("history", "输入型号", "或从列表选择")
    if selected_model:
...
st.markdown("---")
st.subheader("📈 价格趋势查询")
query_model = dual_model_selector("trend", "输入型号", "或从列表选择")
if query_model:

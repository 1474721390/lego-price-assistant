...
st.set_page_config(page_title="乐高报价分析系统", layout="wide")
...
with g1:
    search_global = st.text_input("🔍 全局搜索型号", placeholder="输入数字快速查找")
with g2:
    threshold = get_alert_threshold()
    new_th = st.number_input("⚠️ 涨跌提醒阈值", min_value=1, value=threshold, step=1)
    if new_th != threshold:
...
with st.expander("📝 批量录入报价", expanded=False):
    user_input = st.text_area("每行一个型号", height=220)
...
if not df.empty:
    ...
    if target:
        ...
        with c_buy:
            buy = st.number_input("💚 可收价格", value=rule["buy"])
        with c_sell:
            sell = st.number_input("❤️ 可出价格", value=rule["sell"])
        if st.button("保存心理价位"):

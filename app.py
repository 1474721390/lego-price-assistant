...
def llm_verify(model, price, remark, line):
    ...
    # 构造提示词，明确告诉 AI 型号和价格的区别
    prompt = f"""你是乐高价格识别校验器。
文本：{line}
提取结果：型号={model}，价格={price}，备注={remark}
判断是否真实有效单价，是否提取错误。
规则：
- 乐高官方型号通常是5位数字，首位非0（如 76914、42115）。
- 价格通常是3-4位数字（如 160、1250），极少情况下可能为5位数字（如 20000）。
- 请根据文本上下文判断提取的型号和价格是否正确。如果价格是5位数字，且文本中另有其他5位数字，请优先将5位数字视为型号。
...
with st.expander("📝 批量录入", expanded=True):
    ...
    if st.button("🔍 解析", type="primary", use_container_width=True):
        ...
        # 获取当天已存在的所有记录（使用 Python 日期过滤，避免时区问题）
        all_records = get_all_price_records()
        today = datetime.now(ZoneInfo("Asia/Shanghai")).date()
        today_set = set()
        if not all_records.empty:
            all_records['time_dt'] = pd.to_datetime(all_records['time'], errors='coerce')
            today_mask = all_records['time_dt'].dt.date == today
            today_records = all_records[today_mask]
        ...
        for idx, (key, item) in enumerate(unique_batch.items()):
            ...
            if ok:
                ...
                # 将当前记录加入 today_set，防止同批次内后续相同记录重复
                today_set.add((m, p, r))
            else:
...
if not df.empty:
    ...
    if target:
        ...
        # 直接使用原始时间字符串的前10位作为日期显示
        def format_date(t_str):
            # 如果时间字符串是 "YYYY-MM-DD HH:MM:SS" 格式，直接取前10位

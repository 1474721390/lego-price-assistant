...
def extract_with_regex(line):
    ...
    cleaned = re.sub(r'(好盒|压盒|瑕疵|盒损|烂盒|破盒|纸袋|M袋|S袋|礼袋|有袋|无袋|\+袋|带袋)', '', line)
...
def extract_by_ai(line):
    try:
        prompt = f'''你是乐高报价解析专家，只输出标准JSON，无其他内容。
从文本提取：model(5位型号)、price(单价)、remark(盒况+袋子)。
规则：
1. 只提取5位数字型号
2. remark只保留：好盒/压盒/瑕疵 + 纸袋/M袋/有袋/无袋，组合用+连接
3. 无则返回null
...
'''
        ...
        resp = requests.post("https://open.bigmodel.cn/api/paas/v4/chat/completions", headers=headers, json=payload, timeout=10)
        if resp.status_code == 200:
            data = json.loads(resp.json()["choices"][0]["message"]["content"])
            m = str(data["model"]).strip() if data.get("model") else None
            p = int(data["price"]) if data.get("price") and data["price"] >= 10 else None
            r = str(data.get("remark", "")).strip()
            if m and len(m) ==5 and m.isdigit() and p:
                return m, p, r
    except:
        pass
    return None, None, ""
...
def get_alerts():
    ...
    alerts.sort(key=lambda x: -x["abs_diff"])
    return alerts

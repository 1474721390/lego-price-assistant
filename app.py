...
3. remark：只保留盒况、乐高袋子尺码（SS/XS/S/M/L/XL/XXL等），其他一律不要
输出：{"models":[],"prices":[],"remarks":[]}
'''
        resp = requests.post(
        ...
        for m, p, r in zip(data.get("models", []), data.get("prices", []), data.get("remarks", [])):
            m = str(m).strip()
            if not m.isdigit() or len(m)!=5: continue
            try:
                p = int(p)
                if p<10: continue
            except: continue
...
def extract_by_regex(line):
    ...
    if not line:
        return []
    models = re.findall(r'(?<![0-9])([1-9]\d{4})(?![0-9])', line)
    if not models:
        return []
    # 只取≥10的数字为价格
    prices = []
...
def smart_extract(line):
    ...
    if not final:
        if re.search(r'[1-9]\d{4}', line) or any(w in line for w in ['乐高','盒','袋']):
...
def get_alerts():
    ...
    for m in df["型号"].unique():
        ...
        if abs(d)>=t:
            alerts.append({"model":m,"diff":d,"abs_diff":abs(d),"last":s.iloc[-1]["价格"],
...
def save_batch(records):
    try: return supabase.table("price_records").insert(records).execute()
    except: return None

def update_record(id, data):
    try: return supabase.table("price_records").update(data).eq("id", id).execute()
    except: return None

def delete_record(id):
    try: return supabase.table("price_records").delete().eq("id", id).execute()
    except: return None
...
with col1: search = st.text_input("🔍 搜索型号")
with col2:
    ...
    if st.button("🔍 解析", type="primary", use_container_width=True):
        ...
        for li in txt.strip().splitlines():
            for m,p,r in smart_extract(li):
                if m and p and len(m)==5 and p>=10:
    ...
    if not st.session_state.parse_result.empty:
        ...
        if st.button("💾 修改并保存有效数据", type="primary", use_container_width=True):
            ...
            for _, row in ed.iterrows():
                m = str(row["型号"]).strip()
                p = row["价格"]
                if m and p and "✅" in row["状态"] and len(m)==5 and m.isdigit() and int(p)>=10:
...
if not df.empty:
    ...
    if target:
        if st.button("⭐ 取消收藏" if target in favs else "☆ 收藏"):
            toggle_favorite(target)
            st.rerun()
        rule = get_price_rules().get(target,{"buy":0,"sell":0})
        cb, cs = st.columns(2)
        with cb: b = st.number_input("💚 可收价格", value=rule["buy"])
        with cs: s = st.number_input("❤️ 可出价格", value=rule["sell"])
        if st.button("💾 保存心理价位"):
        ...
        if not md.empty:
            cur = md.iloc[0]["价格"]
            tip = "❤️ 可出货" if s>0 and cur>=s else "💚 可收货" if b>0 and cur<=b else ""
            if tip: st.info(f"当前价 {cur} → {tip}")
        ...
        if st.button("保存修改 & 删除"):
            ...
            for _, row in ed_table[~ed_table["删除"]].iterrows():
                update_record(row["id"],{"model":str(row["型号"]).strip(),"price":int(row["价格"]),"remark":str(row["备注"]).strip()})
            st.success("完成")
            get_clean_data.clear()
            st.rerun()

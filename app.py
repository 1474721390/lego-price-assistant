...
def get_trend_data():
    ...
    if 'remark' not in df.columns:
        df['remark'] = ''
    # 调试输出（仅在首次加载时显示，避免刷屏）
    st.write(f"get_trend_data 返回 {len(df)} 行")
    return df
...
def get_all_price_records():
    ...
    st.write(f"get_all_price_records 返回 {len(df)} 行")
    return df
...
def get_model_list():
    ...
    st.write(f"get_model_list 返回 {len(models)} 个型号: {models[:10]}...")  # 只显示前10个
    return models

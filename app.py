...
def get_trend_data():
    ...
    if 'remark' not in df.columns:
        df['remark'] = ''
    # 调试：打印前10个型号（仅在缓存未命中时执行，不影响性能）
    # st.write(f"get_trend_data 返回 {len(df)} 行，示例型号: {df['型号'].head(10).tolist()}")
    return df
...
def show_trend_chart(model):
    ...
    if df.empty:
        ...
        return
    model = str(model).strip()
    # 诊断：打印前10个型号，帮助确认是否包含该型号
    sample_models = df['型号'].head(10).tolist()
    st.write(f"当前缓存中的前10个型号: {sample_models}")
    st.write(f"正在查询型号: {model}，数据库中有效型号总数: {len(df)}")
    df_model = df[df['型号'] == model].sort_values('时间')

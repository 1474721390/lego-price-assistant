...
# ---------- 组合控件（单个） ----------
def single_model_selector(key_prefix, label="型号选择"):
    """
    单个组合控件：下拉列表包含“手动输入”和所有型号，选择“手动输入”时显示文本输入框。
    返回用户选择的型号（字符串）。
    """
    model_list = get_model_list()
    options = ["手动输入"] + model_list
    selected = st.selectbox(label, options, key=f"{key_prefix}_select")
    if selected == "手动输入":
        manual_input = st.text_input("请输入型号", key=f"{key_prefix}_manual")
...
with st.expander(f"⚠️ 价格趋势预警（{len(alerts)}个型号）", expanded=False):
    if alerts:
        # 使用单个组合控件筛选型号
        filter_model = single_model_selector("alert_filter", "筛选型号（可选）")
...
if all_records_df.empty:
    ...
    selected_model_for_history = single_model_selector("history", "选择要修改的型号")
...
st.markdown("---")
st.subheader("📈 价格趋势查询")
query_model = single_model_selector("trend", "选择或输入型号")
if query_model:

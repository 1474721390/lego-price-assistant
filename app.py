...
def extract_by_regex(line):
    ...
    valid_prices = [p for p in price_candidates if 10 <= p <= 5000]
...
def llm_verify(model, price, remark, line):
    ...
    # 价格合理性检查（不触发 AI 的自动通过条件）
    price_ok = 10 <= price <= 5000
    ...
    if model in latest:
        ...
        if abs(price - last_price) > 50:
            ...
            need_ai = True  # 波动大时需要 AI 复核

    # 如果价格、型号都正常，且波动不大，直接通过，无需 AI
    if price_ok and model_ok and price_change_ok:
        return True, "正常，自动通过"

    # 其他情况（价格异常、型号错误或波动大）才需要 AI 校验

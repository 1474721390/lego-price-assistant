...
def extract_by_ai(line):
    try:
        prompt = f'''从乐高报价文本提取信息，只输出JSON，无多余内容。
字段：
- model：5位数字型号
- price：数字价格（≥10）
- remark：盒况+袋子，如：好盒、压盒、纸袋、M袋、好盒+纸袋
...
def extract_by_regex(line):
    ...
    clean_line = re.sub(r'好盒|压盒|瑕疵|盒损|烂盒|破盒|纸袋|M袋|S袋|礼袋|有袋|无袋|\+袋|带袋|\*|\[|\]|【|】|（|）', '', line)
    
    model_match = re.search(r'(?<!\d)([1-9]\d{4})(?!\d)', clean_line)
    if not model_match:
...
def smart_extract(line):
    ...
    if m and p:
        if 10 <= p <= 50000 and not is_price_abnormal(m, p):
            return m, p, r

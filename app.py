...
if all_records_df.empty:
    ...
    models_list = sorted(all_records_df['model'].astype(str).unique())
    selected_model = st.selectbox("选择要修改的型号", options=["全部"] + models_list, index=0)
    ...
    if filtered_df.empty:
        ...
        st.caption("修改型号或价格后，点击下方按钮更新数据库（注意：id和时间不可修改）")
        edit_df = filtered_df[['id', 'model', 'price', 'time']].copy()
        edit_df['time'] = pd.to_datetime(edit_df['time']).dt.strftime('%Y-%m-%d %H:%M')

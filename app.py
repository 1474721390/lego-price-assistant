...
# ==================== 辅助函数：将列表渲染为网格按钮 ====================
def render_grid_buttons(items, columns=3):
    """
    items: list of (显示文本, 型号, 其他信息可选)
    每行显示 columns 个按钮，点击按钮设置 session_state.selected_model 并滚动
    """
    if not items:
        return
    for i in range(0, len(items), columns):
        ...
        for j, col in enumerate(cols):
            ...
            if idx < len(items):
                item = items[idx]
                # 格式：item 可以是字符串（型号）或元组 (显示文本, 型号)
                if isinstance(item, tuple):
                    label, model = item[0], item[1]
                else:
                    label, model = str(item), str(item)
                if col.button(label, key=f"grid_{model}_{idx}", use_container_width=True):
...
if favs:
    with st.expander("⭐ 我的收藏", expanded=True):
        ...
        render_grid_buttons(fav_items, columns=2)
...
with st.expander("📈 近7日 / 近30日涨幅排行", expanded=False):
    ...
    with c7:
        ...
        if t7:
            ...
            render_grid_buttons(items, columns=1)
        else:
            st.info("暂无数据")
    with c30:
...
with st.expander("📊 价格波动预警", expanded=False):
    ...
    if alerts:
        ...
        with col_up:
            st.subheader("📈 涨价排行")
            if up_list:
                items = [(f"{a['model']} | {a['last']}元 | +{a['abs_diff']}元", a['model']) for a in up_list]
                render_grid_buttons(items, columns=1)
            else:
                st.info("无上涨预警")
        with col_down:
            st.subheader("📉 跌价排行")
            if down_list:
                items = [(f"{a['model']} | {a['last']}元 | -{a['abs_diff']}元", a['model']) for a in down_list]
                render_grid_buttons(items, columns=1)
            else:
                st.info("无下跌预警")
    else:
...
with col_sort1:
    if st.button("📈 价格从高到低", use_container_width=True):
        ...
        if sorted_models:
            items = [(f"{model}  {price}元", model) for model, price in sorted_models]
            render_grid_buttons(items, columns=2)
        else:
            st.info("暂无数据")
with col_sort2:

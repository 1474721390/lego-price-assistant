...
def extract_with_regex(line):
    ...
    # 先清理干扰词，再提取型号和价格
    cleaned = re.sub(r'(好盒|压盒|纸袋|M袋|礼袋|破损|顺丰|普快|包邮|+袋|带袋)', '', line)
    # 型号匹配
    model_match = re.search(r'(?<![0-9])([1-9][0-9]{4})(?![0-9])', cleaned)
    if not model_match:
        return None, None, None
    model = model_match.group(1)
    # 价格匹配（优先行首数字，其次“收”前数字）
...
def parse_with_llm(text):
    ...
    prompt = f"""你是一个乐高报价解析助手。从以下文本中提取出每个乐高产品的官方型号编号、价格和备注信息（如“好盒”、“压盒”、“+纸袋”、“M袋”、“破损”等）。输出一个JSON数组，每个元素包含 model, price, remark 字段。只输出JSON数组。
...
st.markdown("---")
st.caption("数据云端存储，多用户共享。备注自动识别（好盒/压盒/纸袋/M袋等）。可随时导出本地备份。")
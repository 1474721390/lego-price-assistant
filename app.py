...
# ---------- 备注合并函数 ----------
def merge_remark(box_condition, bag_info):
    """
    合并盒况和袋子信息。
    box_condition: 好盒/压盒/瑕疵 等
    bag_info: 纸袋/M袋/礼袋 等
    返回格式： "好盒" / "+纸袋" / "好盒+纸袋"
    """
    if not box_condition and not bag_info:
        return ""
    if box_condition and bag_info:
        return f"{box_condition}+{bag_info}"
    return box_condition or bag_info

# ---------- 正则提取（含备注）----------
def extract_box_and_bag(line):
    """从一行中提取盒况和袋子信息"""
    box = ""
    bag = ""
    # 盒况关键词
    box_keywords = ["好盒", "压盒", "瑕疵", "破损", "烂盒", "破盒", "盒损"]
    for kw in box_keywords:
        if kw in line:
            box = kw
            break
    # 袋子关键词
    bag_keywords = ["纸袋", "M袋", "礼袋", "礼品袋", "M号袋", "S袋", "+袋", "带袋", "无袋"]
...
def extract_with_regex(line):
    ...
    box, bag = extract_box_and_bag(line)
    remark = merge_remark(box, bag)
    # 清理干扰词，保留数字和“收”
    cleaned = re.sub(r'(好盒|压盒|瑕疵|破损|烂盒|破盒|盒损|纸袋|M袋|礼袋|礼品袋|M号袋|S袋|\+袋|带袋|无袋|顺丰|普快|包邮)', '', line)
    # 型号匹配
...
def preprocess_text(text):
    ...
    for line in lines:
        ...
        if re.search(r'\d', line) or "收" in line:
            # 保留所有含数字或“收”的行，但去除明显无关的物流信息
            cleaned = re.sub(r'(普快|顺丰|包邮|顺丰发出|跨越最好|留底|加固|揽收|打包|山东|内蒙|广东|江苏|安徽|浙江|辽宁|吉林|黑龙江|河北|天津|山西|陕西|广西|云南|四川|贵州|福建|重庆|海南|北京|上海|天津|宁夏|青海|甘肃|新疆|西藏|内蒙古)', '', line)
...
def parse_with_llm(text):
    ...
    prompt = f"""你是一个乐高报价解析助手。从以下文本中提取出每个乐高产品的官方型号编号、价格和备注信息。备注需要合并盒况和袋子情况，格式为“盒况+袋子”（如“好盒+纸袋”、“压盒+M袋”），若只有盒况则只写盒况，只有袋子则只写袋子，都没有则留空。输出一个JSON数组，每个元素包含 model, price, remark 字段。只输出JSON数组。

示例：
输入："1180收 乐高 10320 1 好盒 纸袋"
输出：[{{"model": "10320", "price": 1180, "remark": "好盒+纸袋"}}]

输入："880压盒10358声波普快云南"
输出：[{{"model": "10358", "price": 880, "remark": "压盒"}}]

输入："2100收 乐高 10333 1 M袋"
输出：[{{"model": "10333", "price": 2100, "remark": "M袋"}}]
...
st.markdown("---")
st.caption("数据云端存储，多用户共享。备注合并盒况和袋子（如“好盒+纸袋”）。可随时导出本地备份。")
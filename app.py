...
if favs:
    with st.expander("⭐ 我的收藏", expanded=True):
        ...
        render_grid_buttons(fav_items, columns=2, prefix="fav")
...
with col_sort1:
    if st.button("📈 价格从高到低", use_container_width=True):
        ...
        if sorted_models:
            ...
            render_grid_buttons(items, columns=2, prefix="sort_desc")
        else:
            st.info("暂无数据")
with col_sort2:
    if st.button("📉 价格从低到高", use_container_width=True):
        ...
        if sorted_models:
            items = [(f"{model}  {price}元", model) for model, price in sorted_models]
            render_grid_buttons(items, columns=2, prefix="sort_asc")
        else:
            st.info("暂无数据")

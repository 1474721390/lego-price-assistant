...
if alerts:
    with st.expander(f"⚠️ 价格趋势预警（{len(alerts)}个型号）", expanded=False):
        for model, trend, slope in alerts:
            if trend == "上涨":
                st.warning(f"📈 型号 {model} 近期价格整体上涨（斜率 {slope:.2f} 元/天）")
            else:
                st.error(f"📉 型号 {model} 近期价格整体下跌（斜率 {slope:.2f} 元/天）")
    st.markdown("---")
...
if all_records_df.empty:
    ...
    model_list = sorted(all_records_df['model'].astype(str).unique())
    ...
    with col_search:
        search_term = st.text_input("🔍 搜索型号（支持模糊匹配）", 
    ...
    with col_select:
        selected_model = st.selectbox("或从列表中选择型号", 
                                      options=[""] + model_list,
                                      index=0,
                                      key="history_select_box",
                                      on_change=on_select_change)
        if selected_model:
            st.session_state.history_select = selected_model
        else:
            st.session_state.history_select = None
    
    if st.session_state.history_search:
        filtered_df = all_records_df[all_records_df['model'].astype(str).str.contains(st.session_state.history_search, na=False, case=False)]
    elif st.session_state.history_select:
        filtered_df = all_records_df[all_records_df['model'].astype(str) == st.session_state.history_select]
    else:
    ...
    if filtered_df.empty:
        ...
        st.caption("以下为匹配的历史记录，可修改型号/价格，或勾选“删除”后点击“保存修改”。全选可一次勾选所有记录。")
        ...
        if st.button("✅ 全选 / 取消全选"):
            current_delete = edit_data['删除'].tolist()
            if any(not x for x in current_delete):
                edit_data['删除'] = True
            else:
                edit_data['删除'] = False
            st.rerun()
...
if "trend_input" not in st.session_state:
    st.session_state.trend_input = ""
if "trend_select" not in st.session_state:
    st.session_state.trend_select = None

def on_trend_input_change():
    st.session_state.trend_select = None

def on_trend_select_change():
    st.session_state.trend_input = ""

col_left, col_right = st.columns(2)
with col_left:
    trend_input = st.text_input("直接输入型号编号（如10320）", 
                                value=st.session_state.trend_input,
                                key="trend_input_box",
                                on_change=on_trend_input_change)
    st.session_state.trend_input = trend_input.strip()
with col_right:
    models = get_model_list()
    if models:
        trend_select = st.selectbox("或从已记录型号中选择", 
                                    options=[""] + models, 
                                    index=0,
                                    key="trend_select_box",
                                    on_change=on_trend_select_change)
        if trend_select:
            st.session_state.trend_select = trend_select
        else:
            st.session_state.trend_select = None

query_model = None
if st.session_state.trend_input:
    query_model = st.session_state.trend_input
elif st.session_state.trend_select:
    query_model = st.session_state.trend_select

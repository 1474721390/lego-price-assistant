...
def get_favorites():
    res = supabase.table("favorites").select("model").execute()
    return {item["model"] for item in res.data} if res.data else set()

def toggle_favorite(model):
    favs = get_favorites()
    if model in favs:
        supabase.table("favorites").delete().eq("model", model).execute()
    else:
        supabase.table("favorites").insert({"model": model}).execute()
...
def smart_extract(line):
    ...
    if not final:
        if re.search(model_pattern, line) or any(w in line.lower() for w in ['乐高','盒','袋']):

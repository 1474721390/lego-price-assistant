...
with st.expander("📝 批量录入", expanded=True):
    ...
    if st.button("🔍 解析", type="primary", use_container_width=True):
        ...
        save_set = set()  # 记录本次已决定保存的 (model, price, remark)，用于避免批次内重复（虽已去重但加强）
        for idx, (key, item) in enumerate(unique_batch.items()):
            ...
            # 2. 检查本次批次内是否已决定保存完全相同的记录（防止因备注不同导致的重复？但unique_batch已去重，这里加强）
            key_this = (m, p, r.strip())
            if not skip and key_this in save_set:
                skip = True

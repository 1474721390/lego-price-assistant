...
# --- 顶部导航栏 (Tabs) ---
tab1, tab2, tab3, tab4 = st.tabs(["⭐ 我的收藏", "📈 涨跌幅排行", "📊 价格预警", "💰 价格排序"])
...
with tab4:
    ...
    with col_sort1:
        if st.button("📈 价格从高到低", use_container_width=True):
            # 清除之前的选中状态
            st.session_state.selected_model = ""
            st.session_state.sort_result = get_price_sorted_models(order='desc')
            st.session_state.sort_order = 'desc'
    with col_sort2:
        if st.button("📉 价格从低到高", use_container_width=True):
            st.session_state.selected_model = ""
            st.session_state.sort_result = get_price_sorted_models(order='asc')
            st.session_state.sort_order = 'asc'

    # 显示排序结果（如果存在）并添加分页
    if "sort_result" in st.session_state and st.session_state.sort_result:
        items = [(f"{model} | {price}元", model) for model, price in st.session_state.sort_result]
        total_items = len(items)
        default_page_size = 20
        page_size = st.selectbox("每页显示", options=[10, 20, 50], index=1, key="page_size_select")
        total_pages = max(1, (total_items + page_size - 1) // page_size) # 向上取整
        current_page = st.session_state.get("current_page", 1)
        current_page = st.slider("页码", 1, total_pages, current_page, key="page_slider")
        st.session_state.current_page = current_page

        paginated_items = paginate(items, page_size, current_page)
        render_grid_buttons(paginated_items, columns=2, prefix="sort_tab_paged")
        st.caption(f"共 {total_items} 条数据，当前显示第 {current_page} 页，共 {total_pages} 页")
    else:
        st.caption("点击上方按钮生成排序")

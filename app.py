...
if favs:
    with st.expander("⭐ 我的收藏", expanded=True):
        for m in favs:
            ...
            icon = "📈" if d>0 else "📉"
            if st.button(f"【{m}】{icon} {d:+}元 | 当前 {s.iloc[-1]['价格']}元", key=f"fav_{m}", use_container_width=True):
...
with st.expander("📈 近7日 / 近30日 涨幅排行", expanded=False):
    ...
    with c7:
        ...
        for item in t7[:10]:
            m = item["model"]
            if st.button(f"【{m}】{item['diff']:+}元 | {item['last']}元", key=f"t7_{m}", use_container_width=True):
                st.session_state.selected_model = m
                st.rerun()
    with c30:
        ...
        for item in t30[:10]:
            m = item["model"]
            if st.button(f"【{m}】{item['diff']:+}元 | {item['last']}元", key=f"t30_{m}", use_container_width=True):
                st.session_state.selected_model = m
                st.rerun()
...
with st.expander("📊 价格波动预警", expanded=False):
    ...
    with col_up:
        ...
        for a in up_list:
            ...
            m = a["model"]
            btn_label = f"{star} {m} | {a['last']}元 | +{a['abs_diff']}元"
            if st.button(btn_label, key=f"up_{m}", use_container_width=True):
                st.session_state.selected_model = m
                st.rerun()
    with col_down:
        st.subheader("📉 跌价排行")
        for a in down_list:
            ...
            m = a["model"]
            btn_label = f"{star} {m} | {a['last']}元 | -{a['abs_diff']}元"
            if st.button(btn_label, key=f"down_{m}", use_container_width=True):
                st.session_state.selected_model = m
                st.rerun()
...
if not df.empty:
    target = st.selectbox(
        "选择型号",
        [""] + all_models,
        index=(all_models.index(st.session_state.selected_model) + 1) if st.session_state.selected_model in all_models else 0
    )

...
if not df.empty:
    ...
    if target:
        ...
        time_dict = dict(zip(all_records["id"], all_records["time"]))
        ...
        show = model_data[["id","型号","价格","remark"]].copy()
        show["时间"] = show["id"].apply(lambda x: format_time(time_dict.get(x, "")))

...
with st.expander("📝 批量录入", expanded=True):
    ...
    # 输入框
    txt = st.text_area("粘贴内容", height=200, value=st.session_state.txt_content)
    ...
    if st.button("🔍 解析", type="primary"):
        ...
        for li in lines:
            ...
            if m and p:
                ...
                res.append({"型号": m, "价格": p, "备注": r, "状态": "✅ 有效" if ok else "❌ 无效"})
            else:
                res.append({"型号": "", "价格": "", "备注": "", "状态": "❌ 解析失败"})

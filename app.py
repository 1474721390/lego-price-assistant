...
filtered = [m for m in all_models] if search is None else [m for m in all_models if search in m]
...
with st.expander("📊 价格波动预警", expanded=False):
    ...
    with col_down:
        ...
        for a in down_list:
            star = "⭐" if a["is_fav"] else ""
            st.error(f"{star} {a['model']} | {a['last']}元 | -{a['abs_diff']}元")st.divider()

# ------------------------------
# 批量录入 —— 已修复：可编辑型号、价格、备注
# ------------------------------

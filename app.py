...
def get_trend_data():
    ...
    if 'remark' not in df.columns:
        df['remark'] = ''
    st.write(f"get_trend_data 返回 {len(df)} 行，示例型号前10: {df['型号'].head(10).tolist()}")
    return df
...
def get_all_price_records():
    ...
    st.write(f"get_all_price_records 返回 {len(df)} 行，示例型号前10: {df['model'].head(10).tolist()}")
    return df
...
def show_trend_chart(model):
    ...
    if df.empty:
        ...
        return
    model = str(model).strip()
    st.write(f"正在查询型号: {model}，数据库中共有 {len(df)} 条记录")
    df_model = df[df['型号'] == model].sort_values('时间')
...
def get_model_list():
    ...
    st.write(f"get_model_list 返回 {len(models)} 个型号，前10: {models[:10]}")
    return models

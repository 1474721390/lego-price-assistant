...
def extract_by_regex(line):
    ...
    # 1. 把一行里所有纯数字全部抽出来
    all_num_strings = re.findall(r'\d+', line)
    if len(all_num_strings) < 2:
        return None, None, None

    all_nums = []
    for s in all_num_strings:
        try:
    ...
    # 3. 剩下数字里挑价格：排除型号，排除1~9小数量，取10~5000区间最大合理值
    price_candidates = []
    for n in all_nums:
        if str(n) == model:
            continue

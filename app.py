...
def extract_remark(line):
    box_keywords = ["好盒", "压盒", "瑕疵", "盒损", "烂盒", "破盒", "全新", "微压", "微瑕"]
...
def extract_with_regex(line):
    ...
    # 不移除好盒/压盒/顺丰，避免破坏型号
    model_match = re.search(r'(?<!\d)([1-9]\d{4})(?!\d)', line)
    if not model_match:
        return None, None, remark

    model = model_match.group(1)
    # 把型号去掉，再找价格
    line_no_model = line.replace(model, "")
    # 找所有数字，排除5位，取第一个合理价格
    price_matches = re.findall(r'\b\d+\b', line_no_model)
    price = None
...
def extract_by_ai(line):
    try:
        prompt = f'''从乐高报价里提取，只返回JSON，不要多余内容：
文本：{line}
字段：
- model：5位数字型号
- price：数字价格（≥10）
- remark：盒况+袋子，如：好盒、压盒、纸袋

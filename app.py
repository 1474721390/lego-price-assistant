...
# 批量录入
# ------------------------------
with st.expander("📝 批量录入", expanded=False):
    ...
    if st.button("解析", type="primary"):
        if not txt:
            ...
            st.session_state["parse"] = pd.DataFrame(res)

    if "parse" in st.session_state and not st.session_state.parse.empty:
        ed = st.data_editor(st.session_state.parse, use_container_width=True, hide_index=True)
        ...
        with bc1:
            if st.button("保存有效数据"):
                ...
                if ok_list:
                    save_batch(ok_list)
                    st.success(f"保存 {len(ok_list)} 条")
                    del st.session_state["parse"]
                    get_clean_data.clear()
                    st.rerun()
        with bc2:
            if st.button("清空"):
...
if not df.empty:
    ...
    if target:
        ...
        buy = rules.get(target, {}).get("buy",0)
        sell = rules.get(target, {}).get("sell",0)
        cb, cs = st.columns(2)
        with cb:
            b = st.number_input("💚可收价格", value=buy)
        with cs:
            s = st.number_input("❤️可出价格", value=sell)
        if st.button("保存心理价位"):
        ...
        if not model_data.empty:
            ...
            tip = ""
            if s>0 and cur>=s: tip = "❤️ 可出货"
            if b>0 and cur<=b: tip = "💚 可收货"

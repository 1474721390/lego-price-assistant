...
def extract_by_ai(line):
    try:
        prompt = f'''
        从以下文本中提取乐高信息，只返回JSON，不要解释：
        文本：{line}
        输出格式：{{"model":"型号","price":价格,"remark":"备注"}}
        无则返回null
        '''
        resp = requests.post(
        ...
        if not data or not data.get("model") or not data.get("price"):
            return None, None, None
        return str(data["model"]), int(data["price"]), str(data.get("remark", ""))
    except:
        return None, None, None
...
def smart_extract(line):
    ...
    # 3. 正则失败 → 含乐高关键词 → AI兜底
    if any(key in line for key in ["乐高", "LEGO", "Lego", "好盒", "压盒", "瑕疵", "全新"]):
        return extract_by_ai(line)
...
with st.expander("📝 批量录入", expanded=True):
    ...
    if not st.session_state.parse_result.empty:
        ed = st.data_editor(st.session_state.parse_result, use_container_width=True, hide_index=True)
